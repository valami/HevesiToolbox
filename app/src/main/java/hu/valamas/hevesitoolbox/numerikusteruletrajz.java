...
public class numerikusteruletrajz extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        float cx_d =(float) (ax + (b * Math.sin(szog)));
        float cy_d =(float) (ay + (b * Math.cos(szog)));
        double s = (a + b + c) / 2;
        double terulet = Math.sqrt(s * (s - a) * (s - b) * (s - c));
        double mc = (2*terulet)/c;
        ...
        {
            cx = 1.0f;
            cy = -1 + ((cy_d/ (float) (cx))*2) ;
            bx = -1 + (( (float)   b  / (float) (cx))*2) ;
        }
    ...
    public static class Triangle {
        ...
        private float[] vertices = {  // Vertices of the triangle
                cx,  cy, 0.0f, // 0. top
                ax,  ay, 0.0f, // 1. left-bottom
                bx,  by, 0.0f  // 2. right-bottom
        };
